...
open class SCNXYZAxis: SCNNode {
    ...
    private func addXYZAxis(radius: CGFloat, sizeVector: SCNVector3, color: ColorXYZ, name: StringXYZ) {
        ...
        xAxis = SCNAxis.init(type:.X, radius: radius, length: sizeVector.x.cgFloat, color: color.x, name: name.x)
        yAxis = SCNAxis.init(type:.Y, radius: radius, length: sizeVector.y.cgFloat, color: color.y, name: name.y)
        zAxis = SCNAxis.init(type:.Z, radius: radius, length: sizeVector.z.cgFloat, color: color.z, name: name.z)
...
open class SCNAxis: SCNNode {
    ...
    var translateSetup: SCNVector3 = SCNVector3() {
        didSet {
            ...
            axisNameNode.transform = SCNMatrix4Translate(axisNameNode.transform, translateSetup.x * offset , translateSetup.y * (offset + 0.04), translateSetup.z * offset)
        }
    }
}
